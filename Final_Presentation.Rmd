...
- Each hidden state has a probability, which is roughtly estimated based on the relative frequencies of bases covered or not covered by genes, along with a prior estimate of the number of differentially expressed genes.
...
4. Calculate the null test statistics for each region and their averages: $DER \bar{s}^0_{\rho} = \sum_{l \in DER^0_\rho}s^0(l)$.


## Methods - Assess significance of DERs using permutation test (cont.)

- Repeat the permutation percedure $B$ times, and calculate $$p_r = \frac{\sum^B_{b=1}\sum^{P_b}_{\rho=1}I(\bar{s}^0_{\rho=1}>\bar{s_r})}{\sum^B_{b=1}P_b}$$
